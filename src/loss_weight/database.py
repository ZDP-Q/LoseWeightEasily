...
class DatabaseManager:
    ...
    def get_food_info(self, fdc_id: int) -> Optional[Tuple[str, str]]:
        """获取食品基本信息"""
        conn = self.get_connection()
        cursor = conn.cursor()
    ...
    def get_food_calories(self, fdc_id: int) -> Optional[Tuple[float, str]]:
        """获取食品的卡路里信息"""
        conn = self.get_connection()
        cursor = conn.cursor()
    ...
    def get_food_complete_info(self, fdc_id: int) -> Optional[Dict[str, Any]]:
