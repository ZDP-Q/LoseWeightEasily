...
from .models import WeightRecord, WeightRecordCreate, WeightStatistics
...
class WeightTracker:
    ...
    def __init__(self, db_manager: DatabaseManager | None = None, db_path: str | None = None):
        """
        初始化体重跟踪器
    ...
    def db_manager(self) -> DatabaseManager:
        ...
        if self._db_manager is None:
            ...
            self._db_manager = DatabaseManager(db_path=self._db_path)
    ...
    def record_weight(self, weight_kg: float, notes: str = "") -> int:
        ...
        """
        # 使用 Pydantic 验证
        record = WeightRecordCreate(weight_kg=weight_kg, notes=notes)
    ...
    def get_latest_record(self) -> WeightRecord | None:
        """
        获取最新的体重记录

        Returns:
            最新记录的 WeightRecord 模型
        """
        with self.db_manager.get_connection() as conn:
            cursor = conn.cursor()
        ...
        if result:
            return WeightRecord(
                id=result[0], weight_kg=result[1], recorded_at=result[2], notes=result[3] or ""
            )
        return None

    def get_records(self, limit: int = 30) -> list[WeightRecord]:
        """
        获取历史体重记录
        ...
        return [
            WeightRecord(id=row[0], weight_kg=row[1], recorded_at=row[2], notes=row[3] or "")
            for row in results
        ]

    def get_weight_statistics(self) -> WeightStatistics:
        """
        获取体重统计信息
        ...
        with self.db_manager.get_connection() as conn:
            ...
            if total_records == 0:
                return WeightStatistics(total_records=0)
